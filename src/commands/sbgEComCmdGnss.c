 ...
 */
static SbgErrorCode
    sbgEComCmdGnssGetModelId(SbgEComHandle *pHandle, SbgEComGnssModelsStdIds *pModelId, SbgEComCmd cmdId)
{
 ...
 */
static SbgErrorCode
    sbgEComCmdGnssInstallationGet(SbgEComHandle *pHandle, SbgEComGnssInstallation *pGnssInstallation, SbgEComCmd cmdId)
{
    SbgErrorCode           errorCode = SBG_NO_ERROR;
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, cmdId, &receivedPayload, pHandle->cmdDefaultTimeOut);
            ...
            if (errorCode == SBG_NO_ERROR)
            {
                ...
                pGnssInstallation->leverArmSecondaryMode =
                    (SbgEComGnssInstallationMode)sbgStreamBufferReadUint8LE(&inputStream);
 ...
 * \return									SBG_NO_ERROR if the command has been executed successfully.
 */
static SbgErrorCode sbgEComCmdGnssInstallationSet(
    ...
    SbgEComCmd                     cmdId)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, cmdId, sbgStreamBufferGetLinkedBuffer(&outputStream),
            sbgStreamBufferGetLength(&outputStream));
 ...
 *
 * \param[in]	pHandle						A valid sbgECom handle.
 * \param[out]	pAlignConf					Pointer to a SbgEComGnssRejectionConf struct to hold rejection configuration
 * of the gnss module. \param[in]	cmdId						The command identifier to get parameters for a specific
 * GNSS module. \return									SBG_NO_ERROR if the command has been executed successfully.
 */
static SbgErrorCode
    sbgEComCmdGnssGetRejection(SbgEComHandle *pHandle, SbgEComGnssRejectionConf *pRejectConf, SbgEComCmd cmdId)
{
    SbgErrorCode           errorCode = SBG_NO_ERROR;
    SbgEComProtocolPayload receivedPayload;
    uint32_t               trial;
 ...
 *
 * \param[in]	pHandle						A valid sbgECom handle.
 * \param[out]	pAlignConf					Pointer to a SbgEComGnssRejectionConf struct holding rejection configuration
 * for the gnss module. \param[in]	cmdId						The command identifier to set parameters for a specific
 * GNSS module. \return									SBG_NO_ERROR if the command has been executed successfully.
 */
static SbgErrorCode
    sbgEComCmdGnssSetRejection(SbgEComHandle *pHandle, const SbgEComGnssRejectionConf *pRejectConf, SbgEComCmd cmdId)
{
    SbgErrorCode    errorCode = SBG_NO_ERROR;
    uint32_t        trial;
    uint8_t         outputBuffer[64];
