...
SbgErrorCode sbgEComReceiveAnyCmd(
...
SbgErrorCode sbgEComReceiveAnyCmd2(
    SbgEComHandle *         pHandle,
    uint8_t *               pMsgClass,
    uint8_t *               pMsgId,
            ...
            if (sbgEComMsgClassIsALog((SbgEComClass)receivedMsgClass))
            {
                if (pHandle->pReceiveLogCallback)
                {
                    ...
                    errorCode = sbgEComBinaryLogParse(
                        (SbgEComClass)receivedMsgClass, receivedMsgId, sbgEComProtocolPayloadGetBuffer(pPayload),
                        sbgEComProtocolPayloadGetSize(pPayload), &logData);

                    if (errorCode == SBG_NO_ERROR)
                    {
                        pHandle->pReceiveLogCallback(
                            pHandle, (SbgEComClass)receivedMsgClass, receivedMsgId, &logData, pHandle->pUserArg);
...
SbgErrorCode sbgEComReceiveCmd(
    SbgEComHandle *pHandle,
    uint8_t        msgClass,
    uint8_t        msgId,
    void *         pData,
...
SbgErrorCode sbgEComReceiveCmd2(
    SbgEComHandle *         pHandle,
    uint8_t                 msgClass,
    uint8_t                 msgId,
    SbgEComProtocolPayload *pPayload,
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            else if ((receivedMsgClass == SBG_ECOM_CLASS_LOG_CMD_0) && (receivedMsgId == SBG_ECOM_CMD_ACK))
            {
                ...
                sbgStreamBufferInitForRead(
                    &streamBuffer, sbgEComProtocolPayloadGetBuffer(pPayload), sbgEComProtocolPayloadGetSize(pPayload));
...
SbgErrorCode sbgEComSendAck(SbgEComHandle *pHandle, uint8_t msgClass, uint8_t msg, SbgErrorCode cmdError)
{
    ...
    //
    return sbgEComProtocolSend(
        &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_ACK,
