...
SbgErrorCode sbgEComCmdSettingsAction(SbgEComHandle *pHandle, SbgEComSettingsAction action)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SETTINGS_ACTION,
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SETTINGS_ACTION, pHandle->cmdDefaultTimeOut);
...
SbgErrorCode sbgEComCmdExportSettings(SbgEComHandle *pHandle, void *pBuffer, size_t *pSize, size_t maxSize)
{
    ...
    // Call function that handle data transfer
    //
    return sbgEComTransferReceive(
        pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_EXPORT_SETTINGS, pBuffer, pSize, maxSize);
}
