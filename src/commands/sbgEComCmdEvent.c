...
SbgErrorCode sbgEComCmdSyncInGetConf(SbgEComHandle *pHandle, SbgEComSyncInId syncInId, SbgEComSyncInConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_IN_CONF, &syncInIdParam,
            sizeof(syncInIdParam));
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_IN_CONF, &receivedPayload,
...
SbgErrorCode sbgEComCmdSyncInSetConf(SbgEComHandle *pHandle, SbgEComSyncInId syncInId, const SbgEComSyncInConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_IN_CONF,
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_IN_CONF, pHandle->cmdDefaultTimeOut);
...
SbgErrorCode sbgEComCmdSyncOutGetConf(SbgEComHandle *pHandle, SbgEComSyncOutId syncOutId, SbgEComSyncOutConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_OUT_CONF, &syncOutIdParam,
            sizeof(syncOutIdParam));
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_OUT_CONF, &receivedPayload,
                pHandle->cmdDefaultTimeOut);
            ...
            if (errorCode == SBG_NO_ERROR)
            {
                ...
                //
                // Read parameters
                // First is returned the id of the sync, then a reserved field, the output function, polarity and the
                // duration at last.
                //
...
SbgErrorCode
    sbgEComCmdSyncOutSetConf(SbgEComHandle *pHandle, SbgEComSyncOutId syncOutId, const SbgEComSyncOutConf *pConf)
{
    SbgErrorCode    errorCode = SBG_NO_ERROR;
    uint32_t        trial;
    uint8_t         outputBuffer[SBG_ECOM_MAX_BUFFER_SIZE];
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_OUT_CONF,
            sbgStreamBufferGetLinkedBuffer(&outputStream), sbgStreamBufferGetLength(&outputStream));
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_SYNC_OUT_CONF, pHandle->cmdDefaultTimeOut);
