...
SbgErrorCode sbgEComCmdSensorGetMotionProfileId(SbgEComHandle *pHandle, SbgEComMotionProfileStdIds *pModelId)
{
    ...
    errorCode =
        sbgEComCmdGenericGetModelId(pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_MOTION_PROFILE_ID, &modelIdAsUint);
...
SbgErrorCode sbgEComCmdSensorGetInitCondition(SbgEComHandle *pHandle, SbgEComInitConditionConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_INIT_PARAMETERS, NULL, 0);
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_INIT_PARAMETERS, &receivedPayload,
...
SbgErrorCode sbgEComCmdSensorSetInitCondition(SbgEComHandle *pHandle, const SbgEComInitConditionConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_INIT_PARAMETERS,
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_INIT_PARAMETERS, pHandle->cmdDefaultTimeOut);
...
SbgErrorCode sbgEComCmdSensorGetAidingAssignment(SbgEComHandle *pHandle, SbgEComAidingAssignConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_AIDING_ASSIGNMENT, NULL, 0);
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_AIDING_ASSIGNMENT, &receivedPayload,
                pHandle->cmdDefaultTimeOut);
            ...
            if (errorCode == SBG_NO_ERROR)
            {
                ...
                sbgStreamBufferSeek(
                    &inputStream, 4 * sizeof(uint8_t), SB_SEEK_CUR_INC); /*!< Reserved fields to ignore */
...
SbgErrorCode sbgEComCmdSensorSetAidingAssignment(SbgEComHandle *pHandle, const SbgEComAidingAssignConf *pConf)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_AIDING_ASSIGNMENT,
            sbgStreamBufferGetLinkedBuffer(&outputStream), sbgStreamBufferGetLength(&outputStream));
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_AIDING_ASSIGNMENT, pHandle->cmdDefaultTimeOut);
    ...
    float *                     pLeverArm)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_IMU_ALIGNMENT_LEVER_ARM, NULL, 0);
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComReceiveCmd2(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_IMU_ALIGNMENT_LEVER_ARM, &receivedPayload,
                pHandle->cmdDefaultTimeOut);
    ...
    const float *                     pLeverArm)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_IMU_ALIGNMENT_LEVER_ARM,
            sbgStreamBufferGetLinkedBuffer(&outputStream), sbgStreamBufferGetLength(&outputStream));
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            //
            errorCode = sbgEComWaitForAck(
                pHandle, SBG_ECOM_CLASS_LOG_CMD_0, SBG_ECOM_CMD_IMU_ALIGNMENT_LEVER_ARM, pHandle->cmdDefaultTimeOut);
