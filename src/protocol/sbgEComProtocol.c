...
static void sbgEComProtocolDiscardUnusedBytes(SbgEComProtocol *pProtocol)
{
    ...
    if (pProtocol->discardSize != 0)
    {
        ...
        memmove(
            pProtocol->rxBuffer, &pProtocol->rxBuffer[pProtocol->discardSize],
...
static void sbgEComProtocolRead(SbgEComProtocol *pProtocol)
{
    ...
    if (pProtocol->rxBufferSize < sizeof(pProtocol->rxBuffer))
    {
        ...
        errorCode = sbgInterfaceRead(
            pProtocol->pLinkedInterface, &pProtocol->rxBuffer[pProtocol->rxBufferSize], &nrBytesRead,
            sizeof(pProtocol->rxBuffer) - pProtocol->rxBufferSize);
 ...
 */
static SbgErrorCode sbgEComProtocolParseFrame(
    SbgEComProtocol *pProtocol,
    size_t           offset,
    size_t *         pEndOffset,
    uint8_t *        pMsgClass,
    uint8_t *        pMsgId,
                ...
                if (errorCode == SBG_NO_ERROR)
                {
                    ...
                    if (lastByte == SBG_ECOM_ETX)
                    {
                        ...
                        else
                        {
                            ...
                            SBG_LOG_ERROR(
                                errorCode, "invalid CRC, frame:%#" PRIx16 " computed:%#" PRIx16, frameCrc, computedCrc);
                        }
                    }
                    else
                    {
 ...
 */
static SbgErrorCode sbgEComProtocolFindFrame(
    SbgEComProtocol *pProtocol,
    uint8_t *        pMsgClass,
    uint8_t *        pMsgId,
    uint8_t *        pTransferId,
    ...
    size_t *         pSize)
{
    ...
    while (startOffset < pProtocol->rxBufferSize)
    {
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            errorCode = sbgEComProtocolParseFrame(
                pProtocol, offset, &endOffset, pMsgClass, pMsgId, pTransferId, pPageIndex, pNrPages, pBuffer, pSize);
 ...
 * \return									SBG_NO_ERROR if successful.
 */
static SbgErrorCode sbgEComProtocolSendStandardFrame(
    SbgEComProtocol *pProtocol,
    uint8_t          msgClass,
    uint8_t          msgId,
    const void *     pData,
    size_t           size)
{
    ...
    return sbgInterfaceWrite(
        pProtocol->pLinkedInterface, sbgStreamBufferGetLinkedBuffer(&streamBuffer),
        sbgStreamBufferGetLength(&streamBuffer));
}
 ...
 * \return									SBG_NO_ERROR if successful.
 */
static SbgErrorCode sbgEComProtocolSendExtendedFrame(
    SbgEComProtocol *pProtocol,
    uint8_t          msgClass,
    uint8_t          msgId,
    uint8_t          transferId,
 ...
 *											SBG_NOT_READY otherwise.
 */
static SbgErrorCode sbgEComProtocolProcessExtendedFrame(
    SbgEComProtocol *pProtocol,
    uint8_t          msgClass,
    uint8_t          msgId,
    uint8_t          transferId,
...
SbgErrorCode sbgEComProtocolPurgeIncoming(SbgEComProtocol *pProtocol)
{
    ...
    {
        errorCode = sbgInterfaceRead(
            pProtocol->pLinkedInterface, pProtocol->rxBuffer, &numBytesRead, sizeof(pProtocol->rxBuffer));
...
SbgErrorCode
    sbgEComProtocolSend(SbgEComProtocol *pProtocol, uint8_t msgClass, uint8_t msgId, const void *pData, size_t size)
{
    SbgErrorCode errorCode;
    ...
    else
    {
        ...
        if (nrPages <= UINT16_MAX)
        {
            ...
            for (uint16_t pageIndex = 0; pageIndex < nrPages; pageIndex++)
            {
                ...
                errorCode = sbgEComProtocolSendExtendedFrame(
                    pProtocol, msgClass, msgId, transferId, pageIndex, nrPages, &pBuffer[offset], transferSize);
...
SbgErrorCode sbgEComProtocolReceive(
    SbgEComProtocol *pProtocol,
    uint8_t *        pMsgClass,
    uint8_t *        pMsgId,
    void *           pData,
...
SbgErrorCode sbgEComProtocolReceive2(
    SbgEComProtocol *       pProtocol,
    uint8_t *               pMsgClass,
    uint8_t *               pMsgId,
    SbgEComProtocolPayload *pPayload)
{
    ...
    errorCode =
        sbgEComProtocolFindFrame(pProtocol, &msgClass, &msgId, &transferId, &pageIndex, &nrPages, &pBuffer, &size);
    ...
    if (errorCode == SBG_NO_ERROR)
    {
        ...
        {
            errorCode = sbgEComProtocolProcessExtendedFrame(
                pProtocol, msgClass, msgId, transferId, pageIndex, nrPages, pBuffer, size);
...
SbgErrorCode
    sbgEComStartFrameGeneration(SbgStreamBuffer *pOutputStream, uint8_t msgClass, uint8_t msg, size_t *pStreamCursor)
{
...
SbgErrorCode sbgEComFinalizeFrameGeneration(SbgStreamBuffer *pOutputStream, size_t streamCursor)
{
    ...
    if (errorCode == SBG_NO_ERROR)
    {
        ...
        if (payloadSize <= SBG_ECOM_MAX_PAYLOAD_SIZE)
        {
            ...
            //
            frameCrc = sbgCrc16Compute(
                (uint8_t *)sbgStreamBufferGetLinkedBuffer(pOutputStream) + streamCursor + 2, payloadSize + 4);
