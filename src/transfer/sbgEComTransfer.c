...
static SbgErrorCode sbgEComTransferSendInit(SbgEComHandle *pHandle, uint8_t msgClass, uint8_t msg, size_t size)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, msgClass, msg, sbgStreamBufferGetLinkedBuffer(&streamBuffer),
...
static SbgErrorCode sbgEComTransferSendEnd(SbgEComHandle *pHandle, uint8_t msgClass, uint8_t msg)
{
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        //
        errorCode = sbgEComProtocolSend(
            &pHandle->protocolHandle, msgClass, msg, sbgStreamBufferGetLinkedBuffer(&outputStream),
    ...
    size_t         packetSize)
{
    ...
    //
    // Build payload: an SBG_ECOM_TRANSFER_DATA transfer command, the offset from the start of the transfer, the size of
    // the packet the device must send
    //
    sbgStreamBufferInitForWrite(&outputStream, outputBuffer, sizeof(outputBuffer));
    ...
    for (trial = 0; trial < pHandle->numTrials; trial++)
    {
        ...
        {
            //
            // Wait for reponse, the device should respond with a ECOM_TRANSFER_DATA, the offset from the start of the
            // transfer and the data payload If it can not provide the data, it will respond with a NACK
            //
            errorCode = sbgEComReceiveCmd2(pHandle, msgClass, msg, &receivedPayload, pHandle->cmdDefaultTimeOut);
            ...
            if (errorCode == SBG_NO_ERROR)
            {
                ...
                //
                if ((transferCmd == SBG_ECOM_TRANSFER_DATA) && (offset == rcvdOffset) &&
                    (packetSize ==
                     (sbgEComProtocolPayloadGetSize(&receivedPayload) - (sizeof(uint16_t) + sizeof(uint32_t)))))
                {
                    //
                    // Read all the buffer
                    //
...
SbgErrorCode
    sbgEComTransferSend(SbgEComHandle *pHandle, uint8_t msgClass, uint8_t msg, const void *pBuffer, size_t size)
{
    SbgErrorCode   errorCode = SBG_NO_ERROR;
    ...
    if (size <= SBG_ECOM_TRANSFER_MAX_SIZE)
    {
        ...
        if (errorCode == SBG_NO_ERROR)
        {
            ...
            for (i = 0; i < sbgSplitBufferGetSubBufferNbr(&splitBuffer); i++)
            {
                ...
                // Send a sub buffer
                //
                errorCode = sbgEComTransferSendData(
                    pHandle, msgClass, msg, sbgSplitBufferGetSubBuffer(&splitBuffer, i),
                    sbgSplitBufferGetSubBufferOffset(&splitBuffer, i), sbgSplitBufferGetSubBufferSize(&splitBuffer, i));
    ...
    if (errorCode == SBG_NO_ERROR)
    {
        ...
        if (transferSize <= bufferSize)
        {
            ...
            for (i = 0; i < sbgSplitBufferGetSubBufferNbr(&splitBuffer); i++)
            {
                ...
                //
                errorCode = sbgEComTransferReceiveData(
                    pHandle, msgClass, msg, sbgSplitBufferGetSubBuffer(&splitBuffer, i),
                    sbgSplitBufferGetSubBufferOffset(&splitBuffer, i), sbgSplitBufferGetSubBufferSize(&splitBuffer, i));
