...
static void sbgBasicLoggerHandlerWriteImu(const SbgBasicLoggerImu *pImuShort, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%0.12lf\t%0.12lf\t%0.12lf\t%0.12lf\t%0.12lf\t%0.12lf\t%0.3lf\n",
        pImuShort->timestamp, pImuShort->status, pImuShort->deltaVelocity[0], pImuShort->deltaVelocity[1],
        pImuShort->deltaVelocity[2], sbgRadToDegd(pImuShort->deltaAngle[0]), sbgRadToDegd(pImuShort->deltaAngle[1]),
        sbgRadToDegd(pImuShort->deltaAngle[2]), pImuShort->temperature);
...
static void sbgBasicLoggerHandlerWriteImuHeader(void *pStream)
{
    ...
    fprintf(
        pStream,
        "timestamp\tstatus\tdeltaVelX\tdeltaVelY\tdeltaVelZ\tdeltaAngleX\tdeltaAngleY\tdeltaAngleZ\ttemperature\n");
...
static void sbgBasicLoggerHandlerWriteUtc(const SbgLogUtcData *pUtc, void *pStream)
{
    ...
    fprintf(
        pStream,
        "%" PRIu32 "\t%" PRIu16 "\t%" PRIu32 "\t%" PRIu16 "\t%0.2" PRId8 "\t%0.2" PRId8 "\t%0.2" PRId8 "\t%0.2" PRId8
        "\t%0.2" PRId8 "\t%" PRId32 "\n",
        pUtc->timeStamp, pUtc->status, pUtc->gpsTimeOfWeek, pUtc->year, pUtc->month, pUtc->day, pUtc->hour,
        pUtc->minute, pUtc->second, pUtc->nanoSecond);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteStatus(const SbgLogStatusData *pStatus, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%" PRIu16 "\t%" PRIu32 "\t%" PRIu32 "\n", pStatus->timeStamp,
        pStatus->generalStatus, pStatus->comStatus2, pStatus->comStatus, pStatus->aidingStatus);
...
static void sbgBasicLoggerHandlerWriteEkfEuler(const SbgLogEkfEulerData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        sbgRadToDegf(pData->euler[0]), sbgRadToDegf(pData->euler[1]), sbgRadToDegf(pData->euler[2]),
        sbgRadToDegf(pData->eulerStdDev[0]), sbgRadToDegf(pData->eulerStdDev[1]), sbgRadToDegf(pData->eulerStdDev[2]));
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteEkfQuat(const SbgLogEkfQuatData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        pData->quaternion[0], pData->quaternion[1], pData->quaternion[2], pData->quaternion[3],
        sbgRadToDegf(pData->eulerStdDev[0]), sbgRadToDegf(pData->eulerStdDev[1]), sbgRadToDegf(pData->eulerStdDev[2]));
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteEkfNavHeader(void *pStream)
{
    ...
    fprintf(
        pStream, "timestamp\tstatus\tvelN\tvelE\tvelD\tvelStdN\tvelStdE\tvelStdD\tlatitude\tlongitude\taltitude\tlatitu"
                 "deStd\tlongitudeStd\taltitudeStd\tundulation\n");
    fprintf(
        pStream,
        "(us)\t(NA)\t(m.s-1)\t(m.s-1)\t(m.s-1)\t(m.s-1)\t(m.s-1)\t(m.s-1)\t(deg)\t(deg)\t(m)\t(m)\t(m)\t(m)\t(m)\n");
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteEkfNav(const SbgLogEkfNavData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
        pData->timeStamp, pData->status, pData->velocity[0], pData->velocity[1], pData->velocity[2],
        pData->velocityStdDev[0], pData->velocityStdDev[1], pData->velocityStdDev[2], pData->position[0],
        pData->position[1], pData->position[2], pData->positionStdDev[0], pData->positionStdDev[1],
        pData->positionStdDev[2], pData->undulation);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteShipMotionHeader(void *pStream)
{
    assert(pStream);
    fprintf(
        pStream, "timestamp\tstatus\tsurge\tsway\theave\tsurgeVel\tswayVel\theaveVel\tsurgeAccel\tswayAccel\theaveAccel"
                 "\theavePeriod\n");
...
static void sbgBasicLoggerHandlerWriteShipMotion(const SbgLogShipMotionData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n", pData->timeStamp, pData->status,
        pData->shipMotion[0], pData->shipMotion[1], pData->shipMotion[2], pData->shipVel[0], pData->shipVel[1],
        pData->shipVel[2], pData->shipAccel[0], pData->shipAccel[1], pData->shipAccel[2], pData->mainHeavePeriod);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteGpsVel(const SbgLogGpsVel *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp,
        pData->status, pData->timeOfWeek, pData->velocity[0], pData->velocity[1], pData->velocity[2],
        pData->velocityAcc[0], pData->velocityAcc[1], pData->velocityAcc[2], pData->course, pData->courseAcc);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteGps(const SbgLogGpsPos *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%lf\n", pData->timeStamp, pData->timeOfWeek, pData->status,
        pData->altitude);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteGpsHdt(const SbgLogGpsHdt *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%" PRIu32 "\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        pData->timeOfWeek, pData->heading, pData->headingAccuracy, pData->pitch, pData->pitchAccuracy, pData->baseline);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteGpsSat(const SbgLogSatGroupData *pData, void *pStream)
{
    ...
    fprintf(
        pStream,
        "%10" PRIu32 " us | Satellites: %3zu ====================================================================\n",
        pData->timeStamp, pData->nrSatellites);
    ...
    for (satelliteIdx = 0; satelliteIdx < pData->nrSatellites; satelliteIdx++)
    {
        ...
        fprintf(
            pStream,
            "            [ID: %2" PRIu8 " %-8s]  Elevation: %3" PRIi8 "%c  Azimuth: %3" PRIi16
            "  nrSignals: %2zu  %-10s%-10s\n",
            pSatEntry->id, sbgLogSatDataGetConstellationIdAsStr(pSatEntry), pSatEntry->elevation, elevationStatusChr,
            pSatEntry->azimuth, pSatEntry->nrSignals, sbgLogSatDataGetHealthStatusAsStr(pSatEntry),
        ...
        for (signalIdx = 0; signalIdx < pSatEntry->nrSignals; signalIdx++)
        {
            ...
            fprintf(
                pStream, "                %-14s    SNR: %3s    %-10s    %-10s\n",
                sbgLogSatSignalDataGetSignalIdAsStr(pSignalEntry), snrStr,
...
static void sbgBasicLoggerHandlerWriteOdometer(const SbgLogOdometerData *pData, void *pStream)
{
    ...
    fprintf(pStream, "%" PRIu32 "\t%" PRIu16 "\t%lf\n", pData->timeStamp, pData->status, pData->velocity);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteDvl(const SbgLogDvlData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        pData->velocity[0], pData->velocity[1], pData->velocity[2], pData->velocityQuality[0],
        pData->velocityQuality[1], pData->velocityQuality[2]);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteAir(const SbgLogAirData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        pData->pressureAbs, pData->altitude, pData->pressureDiff, pData->trueAirspeed, pData->airTemperature);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteUsbl(const SbgLogUsblData *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", pData->timeStamp, pData->status,
        pData->latitude, pData->longitude, pData->depth, pData->latitudeAccuracy, pData->longitudeAccuracy,
        pData->depthAccuracy);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteDepth(const SbgLogDepth *pData, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%lf\t%lf\n", pData->timeStamp, pData->status, pData->pressureAbs,
        pData->altitude);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteEvent(const SbgLogEvent *pEvent, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%" PRIu16 "\t%" PRIu16 "\t%" PRIu16 "\t%" PRIu16 "\n", pEvent->timeStamp,
        pEvent->status, pEvent->timeOffset0, pEvent->timeOffset1, pEvent->timeOffset2, pEvent->timeOffset3);
    fflush(pStream);
}
...
static void sbgBasicLoggerHandlerWriteMag(const SbgLogMag *pMag, void *pStream)
{
    ...
    fprintf(
        pStream, "%" PRIu32 "\t%" PRIu16 "\t%f\t%f\t%f\t%f\t%f\t%f\n", pMag->timeStamp, pMag->status,
        pMag->magnetometers[0], pMag->magnetometers[1], pMag->magnetometers[2], pMag->accelerometers[0],
        pMag->accelerometers[1], pMag->accelerometers[2]);
    fflush(pStream);
}

static SbgErrorCode sbgBasicLoggerHandlerGetUtcTimeFromTimeStamp(
    uint32_t       timeStamp,
    SbgLogUtcData *pLastUtcData,
    struct tm *    pRefUtcTimestamp,
    uint32_t *     pUs)
{
 ...
 */
static void sbgBasicLoggerCommonHandlerConstruct(
    SbgBasicLoggerCommonHandler *pHandler,
    bool                         consoleEnabled,
    ...
    bool                      writeHeader)
{
    ...
    sbgBasicLoggerCommonHandlerConstruct(
        &pHandler->commonHandler, consoleEnabled, consoleDecimation, writingFile, pPathStr, fileDecimation, writeHeader,
        SBG_ECOM_CLASS_LOG_ECOM_0, SBG_ECOM_LOG_IMU_SHORT);
    ...
    bool                      writeHeader)
{
    ...
    sbgBasicLoggerCommonHandlerConstruct(
        &pHandler->commonHandler, consoleEnabled, consoleDecimation, writingFile, pPathStr, fileDecimation, writeHeader,
        SBG_ECOM_CLASS_LOG_ECOM_1, SBG_ECOM_LOG_FAST_IMU_DATA);
    ...
    const char *               pPathStr)
{
    ...
    assert(!sbgBasicLoggerCommonHandlerInitialized(&pHandler->commonHandler));
    sbgBasicLoggerCommonHandlerConstruct(
        &pHandler->commonHandler, consoleEnabled, 0, writingFile, pPathStr, 0, 0, SBG_ECOM_CLASS_LOG_ECOM_0,
        SBG_ECOM_LOG_DIAG);
}
    ...
    bool                      writeHeader)
{
    ...
    assert(!sbgBasicLoggerCommonHandlerInitialized(&pHandler->commonHandler));
    sbgBasicLoggerCommonHandlerConstruct(
        &pHandler->commonHandler, consoleEnabled, 0, writingFile, pPathStr, 0, writeHeader, SBG_ECOM_CLASS_LOG_ECOM_0,
