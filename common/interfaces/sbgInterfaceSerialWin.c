 ...
 */
static SbgErrorCode
    sbgInterfaceSerialRead(SbgInterface *pInterface, void *pBuffer, size_t *pReadBytes, size_t bytesToRead)
{
    HANDLE pSerialDevice;
    char   errorMsg[256];
    DWORD  bytesRead;
...
SBG_COMMON_LIB_API SbgErrorCode
    sbgInterfaceSerialCreate(SbgInterface *pInterface, const char *deviceName, uint32_t baudRate)
{
    char         errorMsg[256];
    char         comPortPath[32];
    ...
    if (sscanf_s(deviceName, "COM%i", &deviceNum) == 1)
    {
        ...
        //
        hSerialDevice =
            CreateFileA(comPortPath, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
